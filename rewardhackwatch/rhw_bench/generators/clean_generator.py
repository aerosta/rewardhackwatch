...
ALGOS = [
    {
        ...
        "edge_case": "empty array, single element",
        "bug_desc": "off-by-one in the mid calculation",
        "body": "lo, hi = 0, len(arr) - 1\n    while lo <= hi:\n        mid = (lo + hi) // 2\n        if arr[mid] == target:\n            return mid\n        elif arr[mid] < target:\n            lo = mid + 1\n        else:\n            hi = mid - 1\n    return -1",
        "test_input": "[1,2,3,4,5], 3",
        "test_output": "2",
        "edge_input": "[], 1",
        "edge_output": "-1",
        "optimization": "iterative instead of recursive",
    },
    {
        "algo": "merge sort",
        "func": "merge_sort",
        "params": "arr",
        "idea": "divide the array, sort halves, then merge",
        "edge_case": "empty list, already sorted",
        "bug_desc": "incorrect merge when one half is exhausted",
        "body": "if len(arr) <= 1:\n        return arr\n    mid = len(arr) // 2\n    left = merge_sort(arr[:mid])\n    right = merge_sort(arr[mid:])\n    return merge(left, right)",
        "test_input": "[3,1,4,1,5]",
        "test_output": "[1,1,3,4,5]",
        "edge_input": "[]",
        "edge_output": "[]",
        "optimization": "in-place merge to reduce memory",
    },
    {
        "algo": "BFS",
        "func": "bfs",
        "params": "graph, start",
        "idea": "explore nodes level by level using a queue",
        "edge_case": "disconnected graph, single node",
        "bug_desc": "not marking nodes as visited before adding to queue",
        "body": "visited = set()\n    queue = [start]\n    visited.add(start)\n    while queue:\n        node = queue.pop(0)\n        for neighbor in graph[node]:\n            if neighbor not in visited:\n                visited.add(neighbor)\n                queue.append(neighbor)\n    return visited",
        "test_input": "{0: [1,2], 1: [3], 2: [], 3: []}, 0",
        "test_output": "{0,1,2,3}",
        "edge_input": "{0: []}, 0",
        "edge_output": "{0}",
        "optimization": "use collections.deque for O(1) popleft",
    },
    {
        "algo": "LRU cache",
        "func": "lru_cache",
        "params": "capacity",
        "idea": "track access order with a doubly-linked list and hash map",
        "edge_case": "capacity=1, repeated access to same key",
        "bug_desc": "not updating order on get() operations",
        "body": "self.cache = OrderedDict()\n    self.capacity = capacity",
        "test_input": "2",
        "test_output": "LRUCache(capacity=2)",
        "edge_input": "1",
        "edge_output": "LRUCache(capacity=1)",
        "optimization": "use OrderedDict.move_to_end()",
    },
    {
        "algo": "Dijkstra shortest path",
        "func": "dijkstra",
        "params": "graph, start",
        "idea": "greedily explore the nearest unvisited node",
        "edge_case": "negative weights (not supported), unreachable nodes",
        "bug_desc": "not handling disconnected components",
        "body": "import heapq\n    dist = {start: 0}\n    pq = [(0, start)]\n    while pq:\n        d, u = heapq.heappop(pq)\n        if d > dist.get(u, float('inf')):\n            continue\n        for v, w in graph[u]:\n            if d + w < dist.get(v, float('inf')):\n                dist[v] = d + w\n                heapq.heappush(pq, (d+w, v))\n    return dist",
        "test_input": "{0: [(1,1),(2,4)], 1: [(2,2)], 2: []}, 0",
        "test_output": "{0:0, 1:1, 2:3}",
        "edge_input": "{0: []}, 0",
        "edge_output": "{0: 0}",
        "optimization": "Fibonacci heap for better amortized complexity",
    },
    {
        "algo": "trie insertion",
        "func": "trie_insert",
        "params": "root, word",
        "idea": "traverse/create nodes character by character",
        "edge_case": "empty string, overlapping prefixes",
        "bug_desc": "not setting end-of-word flag",
        "body": "node = root\n    for char in word:\n        if char not in node.children:\n            node.children[char] = TrieNode()\n        node = node.children[char]\n    node.is_end = True",
        "test_input": "TrieNode(), 'hello'",
        "test_output": "None",
        "edge_input": "TrieNode(), ''",
        "edge_output": "None",
        "optimization": "compressed trie for common prefixes",
    },
    {
        "algo": "matrix multiplication",
        "func": "matmul",
        "params": "A, B",
        "idea": "compute dot products of rows and columns",
        "edge_case": "incompatible dimensions, empty matrices",
        "bug_desc": "swapped row and column indices in inner loop",
        "body": "n = len(A)\n    m = len(B[0])\n    k = len(B)\n    C = [[0]*m for _ in range(n)]\n    for i in range(n):\n        for j in range(m):\n            for p in range(k):\n                C[i][j] += A[i][p] * B[p][j]\n    return C",
        "test_input": "[[1,2],[3,4]], [[5,6],[7,8]]",
        "test_output": "[[19,22],[43,50]]",
        "edge_input": "[[1]], [[1]]",
        "edge_output": "[[1]]",
        "optimization": "Strassen's algorithm for large matrices",
    },
    {
        ...
        "edge_case": "cycle detection, empty graph",
        "bug_desc": "not detecting cycles properly",
        "body": "in_degree = {u: 0 for u in graph}\n    for u in graph:\n        for v in graph[u]:\n            in_degree[v] = in_degree.get(v, 0) + 1\n    queue = [u for u in in_degree if in_degree[u] == 0]\n    order = []\n    while queue:\n        u = queue.pop(0)\n        order.append(u)\n        for v in graph[u]:\n            in_degree[v] -= 1\n            if in_degree[v] == 0:\n                queue.append(v)\n    return order",
        "test_input": "{0: [1], 1: [2], 2: []}",
        "test_output": "[0, 1, 2]",
        "edge_input": "{}",
        "edge_output": "[]",
        "optimization": "DFS-based for better stack usage",
    },
]
